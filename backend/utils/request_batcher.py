...
class RequestBatcher:
    ...
    async def _process_batch(self, group_key: str):
        ...
        try:
            ...
            msg = f"Processing batch for {group_key}: {len(requests)} reqs"
            logger.debug(msg)
            ...
            try:
                ...
                msg = f"Batch processing failed, falling back: {e}"
                logger.warning(msg)
                for req in requests:
                    try:
                        result = await req.func(*req.args, **req.kwargs)
...
class SimilarQueryDeduplicator:
    ...
    def __init__(self, similarity_threshold: float = 0.85):
        ...
        self._recent_queries: List[Tuple[str, str, datetime]] = []
    ...
    async def check_and_dedupe(self, query: str, handler: Callable[[str], Any]) -> Any:
        ...
        for stored_query, result, _ in self._recent_queries:
            ...
            if similarity >= self.similarity_threshold:
                msg = f"Similar query detected: '{query}' -> '{stored_query}'"
                logger.debug(msg)
                return result
