...
class RateLimiter:
    ...
    async def check(self, client_id: str, endpoint: str) -> bool:
        ...
        try:
            ...
            burst_current = int(
                burst_current.decode('utf-8')
                if isinstance(burst_current, bytes)
                else burst_current
            ) if burst_current else 0
            ...
            if current is None:
                ...
                await client.setex(burst_key, 10, 1)
                return True
            ...
            if count >= policy["max_requests"]:
                ...
                logger.warning(
                    f"限流触发: {client_id}, 端点: {endpoint}, 剩余时间: {ttl}s"
                )
                return False
