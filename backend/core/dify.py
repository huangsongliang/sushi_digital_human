...
class DifyIntegration:
    ...
    def update_application(self, app_id: str, name: Optional[str] = None, description: Optional[str] = None) -> Dict[str, Any]:
        """更新应用信息"""
        url = f"{self._base_url}/apps/{app_id}"
        payload = {}
    ...
    def chat_completion(self, app_id: str, message: str, user_id: str = "default") -> Dict[str, Any]:
        ...
        try:
            response = requests.post(url, headers=self.get_headers(), json=payload, stream=True)
            response.raise_for_status()
