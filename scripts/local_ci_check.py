...
def main():
    ...
    all_passed = True
    steps = [
        ("代码风格检查 (Flake8)", "uv run flake8 backend/ --max-line-length=120 --ignore=E501,W503,W291,W293,F401,F841,E302,F821"),
    ...
    print_header("检查结果总结")
    if all_passed:
        print(f"{Colors.GREEN}{Colors.BOLD}🎉 所有检查通过！可以安全提交代码。{Colors.RESET}")
    else:
        print(f"{Colors.RED}{Colors.BOLD}⚠️  部分检查失败，请修复问题后再提交。{Colors.RESET}")
    print()
