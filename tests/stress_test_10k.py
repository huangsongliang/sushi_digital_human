...
class StressTester:
    ...
    async def run_concurrent_load_test(
        ...
        for target_users, phase_duration in phases:
            ...
            tasks = [self.run_user_session(client, phase_duration) for client in active_clients]
            await asyncio.gather(*tasks)
    ...
    def print_result(self, result: TestResult):
        ...
        print(f"  - 成功请求: {result.success_count:,} ({result.success_rate * 100:.2f}%)")
        ...
        print(f"  - P95 响应时间: {'✅ 通过' if p95_pass else '❌ 未通过'} "
              f"(目标 ≤ {TARGET_P95_RESPONSE_TIME}s, 实际 {result.p95_response_time:.3f}s)")
        print(f"  - 错误率: {'✅ 通过' if error_rate_pass else '❌ 未通过'} "
              f"(目标 ≤ {TARGET_ERROR_RATE * 100}%, 实际 {result.error_rate * 100:.2f}%)")
        print(f"  - 吞吐量: {'✅ 通过' if throughput_pass else '❌ 未通过'} "
