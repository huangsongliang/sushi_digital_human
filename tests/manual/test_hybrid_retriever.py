...
def test_hybrid_retriever():
    ...
    for query in test_queries:
        ...
        bm25_results = retriever.search(
            query, top_k=3, use_vector=False, use_rerank=False
        )
        for i, result in enumerate(bm25_results, 1):
            print(
                f"  {i}. {result.get('content', '')[:50]}... (score: {result.get('score', 0):.4f})"
            )

        # 测试纯向量检索
        print("\n[向量检索]:")
        vector_results = retriever.search(
            query, top_k=3, use_bm25=False, use_rerank=False
        )
        for i, result in enumerate(vector_results, 1):
            print(
                f"  {i}. {result.get('content', '')[:50]}... (distance: {result.get('distance', 0):.4f})"
            )

        # 测试混合检索（无重排序）
        print("\n[混合检索 (RRF融合)]:")
        hybrid_results = retriever.search(
            query, top_k=3, use_bm25=True, use_vector=True, use_rerank=False
        )
        for i, result in enumerate(hybrid_results, 1):
            print(
                f"  {i}. {result.get('content', '')[:50]}... (RRF score: {result.get('rrf_score', 0):.4f})"
            )

        # 测试完整混合检索（含重排序）
        print("\n[完整混合检索 (RRF + 重排序)]:")
        full_results = retriever.search(
            query, top_k=3, use_bm25=True, use_vector=True, use_rerank=True
        )
        for i, result in enumerate(full_results, 1):
            print(f"  {i}. {result.get('content', '')[:50]}...")
            print(
                f"     RRF: {result.get('rrf_score', 0):.4f}, 重排: {result.get('rerank_score', 0):.4f}"
            )
