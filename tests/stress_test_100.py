...
class ResourceMonitor:
    ...
    def sample(self) -> ResourceMetrics:
        ...
        disk_io_write_mb = (disk_io.write_bytes - self._start_disk_io[1]) / (1024 * 1024)
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_network_sent_mb": statistics.mean([m.network_sent_mb for m in self.metrics]),
            "avg_network_recv_mb": statistics.mean([m.network_recv_mb for m in self.metrics]),
...
class StressTester:
    ...
    async def run_concurrent_load_test(
        ...
        # 预热任务
        warmup_tasks = [self.run_user_session(client, warmup_duration) for client in self.clients]
        ...
        while self.running and (time.time() - phase_start) < duration:
            ...
            print(f"\r        进度: {progress:5.1f}% ({int(elapsed)}/{duration}秒)", end='', flush=True)
        ...
        for client in self.clients:
            ...
            self.global_metrics.error_messages.extend(client.errors[:10])  # 只保留前10个错误
        ...
        result = TestResult(
            ...
            avg_response_time=statistics.mean(self.global_metrics.response_times) if self.global_metrics.response_times else 0,
            min_response_time=min(self.global_metrics.response_times) if self.global_metrics.response_times else 0,
            max_response_time=max(self.global_metrics.response_times) if self.global_metrics.response_times else 0,
...
async def main():
    ...
    try:
        ...
        # 保存报告到文件
        report_filename = f"tests/report_100users_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
